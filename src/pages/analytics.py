...
def render_analytics_page() -> None:
    ...
    st.caption(
        "↑ Revenue and subscriber figures above are independent of the churn window setting. "
        f"The churn breakdown below uses the **{churn_window}-day** window."
    )
    ...
    with col_churn:
        ...
        cc1.metric(
            "Customers Lost",
            f"{label_result.n_churned:,}",
            delta=f"-{label_result.churn_rate:.1%}",
            delta_color="inverse",
            help=f"Customers with no activity in the last {churn_window} days.",
        )
        cc2.metric(
            "Customers Retained",
            f"{label_result.n_active:,}",
            delta=f"+{1 - label_result.churn_rate:.1%}",
            delta_color="normal",
            help=f"Customers with at least one transaction in the last {churn_window} days.",
        )
        st.caption(
            f"Churn window: {churn_window} days · "
            f"As of: {label_result.reference_date.date()}"
        )

    with col_rfm:
        st.markdown("#### Customer Behavior")
        st.caption("How recently, how often, and how much each customer spends on average.")
        if rfm_result is not None:
            feat = rfm_result.features
            rc1, rc2 = st.columns(2)
            rc1.metric(
                "Avg Days Since Last Purchase",
                f"{feat['recency_days'].mean():.0f} days",
                help="Lower means customers are buying more recently — a sign of healthy engagement.",
            )
            rc2.metric(
                "Avg Number of Purchases",
                f"{feat['frequency'].mean():.1f}×",
                help="How many payments the average customer has made over the full history.",
            )
            rc1.metric(
                "Avg Total Spend",
                f"${feat['monetary_total'].mean():,.0f}",
                help="Total revenue generated by the average customer across all their transactions.",
            )
            rc2.metric(
                "Avg Order Value",
                f"${feat['aov'].mean():.2f}",
                help="Average amount per individual transaction.",
            )
        else:
            st.info("Not enough customers to compute customer behavior metrics.", icon="ℹ️")
